...
export type WiredProps<P extends {} = {}, Q extends OperationTypes = {}> = P & {
  CSN: boolean;
...
export function Wire<
  P extends {},
  Queries extends OperationTypes,
  ServerSideProps
>(
      ...
      return opts.variablesFromContext
        ...
        : Object.fromEntries(
            Object.keys(queries).map((key) => [
              key,
              defaultVariablesFromContext(router),
            ])
          );
    }, [router]);
...
>(
  ...
  const variables = opts.variablesFromContext
    ...
    : Object.fromEntries(
        Object.keys(queries).map((key) => [
          key,
          defaultVariablesFromContext(ctx),
        ])
      );

  const preloadedQueries = Object.fromEntries(
    Object.entries(queries).map(([key, query]) => [
      key,
      loadQuery(env, query, variables[key]),
    ])
  );

  await Promise.allSettled(
    Object.entries(preloadedQueries).map(([key, preloadedQuery]) =>
      ensureQueryFlushed(preloadedQuery)
    )
  );
